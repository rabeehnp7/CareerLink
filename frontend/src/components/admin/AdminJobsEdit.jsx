...
function AdminJobsEdit() {
    ...
    const {id}=useParams()
    useEffect(()=>{
       const Job= allAdminJobs.filter((job)=>{
           return job._id==id
        })
        if(Job){
            setInput({
                ...
                experience: Job[0].experience || "",
                positions: Job[0].positions || 0,
                jobType: Job[0].jobType || "",
                location: Job[0].location || "",
                companyId: Job[0].company._id || ""
            });
        }
    },[allAdminJobs,id])
    ...
    const submitHandler=async(e)=>{
        ...
        try {
            const res=await axios.post(`${JOB_API_ENDPOINT}/update/${id}`,formData,{
                headers:{
